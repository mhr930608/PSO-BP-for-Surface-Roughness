...
disp(['R2 for the training set£º', num2str(R10)])
disp(['R2 for the test set£º', num2str(R20)])

%  MAE (Mean absolute error) 
mae10 = sum(abs(T0_sim1 - T_train), 2)' ./M  ;
mae20 = sum(abs(T0_sim2 - T_test ), 2)' ./N ;
avmae0= sum(abs(T0_sim - T), 2)' ./(M+N) ;
disp(['MAE for the training set£º', num2str(mae10)])
disp(['MAE for the test set£º', num2str(mae20)])
disp(['Total MAE for BPNN£º', num2str(avmae0)])

%  MAPE (Mean absolute percentage error)
mape10       = sum(abs(error0_train), 2)' ./ M;
mape20       = sum(abs(error0_test), 2)' ./ N;
avmape0      = sum(abs((T-T0_sim)./T), 2)' ./ (M+N);
disp(['MAPE for the training set£º', num2str(mape10)])
disp(['MAPE for the test set£º', num2str(mape20)])
...
disp(['MSE for the training set£º', num2str(rmse10)])
disp(['MSE for the test set£º', num2str(rmse20)])
disp(['Total MSE for BPNN£º', num2str(avmse0)])
fprintf('\n')
